/* ==========================================================
   0) STORAGE CHECK — warns immediately (instead of silently
   losing data) if this browser/context can't actually save.
   ========================================================== */
if (typeof checkStorageAndWarn === 'function') checkStorageAndWarn();

/* ==========================================================
   1) SPLIT-OPEN INTRO — now with a bright flash burst timed
   to the reveal, for a stronger opening moment.
   ========================================================== */
...
/* ==========================================================
   1a) HERO VIDEO — loaded from whatever the owner uploaded on
   admin.html (stored in localStorage), instead of a hardcoded
   assets/hero.mp4 file. Falls back to the placeholder <source>
   already in the HTML if nothing has been uploaded yet, and
   respects the trim in/out points + fit + brightness the owner
   chose in the admin panel.
   ========================================================== */
(function setupHeroVideo() {
  try {
    ...
    const config = typeof getHeroVideo === 'function' ? getHeroVideo() : null;
...
/* ==========================================================
   1b) LIGHTNING / SPARK EFFECTS — jagged electric bolts flash
   across the hero. A burst fires timed to the split-intro
   doors opening, then ambient strikes keep firing at random
   intervals for as long as the hero section is on screen.
   Replaces the old red glow with a flickering white/ice-blue
   charge. Pure SVG generated on the fly, no image assets.
   ========================================================== */
...
function spawnBolt(opts = {}) {
  ...
  // Occasional side-branch off the main bolt, like real lightning
...
// Burst timed to land right as the split-intro doors open
let ambientBoltTimer = null;
try {
  setTimeout(() => lightningBurst(3), 550);

  // Ambient strikes for as long as the hero is actually on screen
...
/* ==========================================================
   2) 3D LETTER REVEAL for the hero title, each letter now
   flips in AND flickers an electric spark as it lands.
   ========================================================== */
  ...
  try {
    ...
    setTimeout(() => {
      letters.forEach(l => {
        ...
        // Once the landing flourish is done, switch to the slow ambient pulse
...
/* ==========================================================
   2b) GLITCH HOVER on the nav wordmark — a quick RGB channel
   slip, then it settles back to the normal chrome shimmer.
   ========================================================== */
...
if (navMark) {
  ...
  navMark.addEventListener('mouseenter', () => {
    ...
    void navMark.offsetWidth; // restart the animation if hovered again quickly
...
/* ==========================================================
   3) MARQUEE — now a pure CSS animation (see styles.css),
   which is far cheaper on phones than a setInterval loop
   updating a transform 40 times a second forever.
   ========================================================== */


/* ==========================================================
   4) ADD-TO-CART SOUND
   A tiny synthesized "pop" using the Web Audio API — no sound
   file needed, works the instant the button is clicked.
   ========================================================== */
let audioCtx = null;
...
/* ==========================================================
   ...
   Clones the clicked product's current photo, then animates
   it flying up into the cart badge in the nav.
   ========================================================== */
...
/* ==========================================================
   6) RENDER PRODUCTS — photo carousel per card, glow + tilt
   hover motion, click-through to the full lightbox.
   ========================================================== */
const grid = document.getElementById('productGrid');

function productPhotoHTML(images, mainOnly) {
  if (!images.length) {
    return `<div class="product-card__img product-card__img--empty"></div>`;
  }
...
function getVisibleProducts() {
  ...
  else visible.sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0)); // newest first
...
function renderProducts() {
  ...
  grid.innerHTML = products.map(p => {
    ...
    const stockNote = stock === Infinity
      ? ''
      : outOfStock
        ? `<p class="stock-note stock-note--out">Out of stock</p>`
        : stock <= 5
          ? `<p class="stock-note stock-note--low">${stock} left</p>`
          : '';
...
function attachCardBehavior() {
  ...
  grid.querySelectorAll('.product-card').forEach(card => {
    ...
    // Light hover tick + a subtle lift, only on devices with real hover
    card.addEventListener('mouseenter', () => {
      if (typeof playHoverTick === 'function') playHoverTick();
    });
    ...
    // Click the photo (not the arrows/dots/button) to open the full lightbox
    if (imgWrap) {
      imgWrap.addEventListener('click', () => openProductModal(id, current));
    }
    ...
    if (wishlistBtn) {
      wishlistBtn.addEventListener('click', (e) => {
        ...
        wishlistBtn.innerHTML = nowSaved ? '&#9829;' : '&#9825;';
        if (typeof showToast === 'function') showToast(nowSaved ? 'Saved to wishlist' : 'Removed from wishlist');
      });
    }

    // Glow + 3D tilt that follows the cursor
    ...
    if (addBtn) {
      addBtn.addEventListener('click', () => {
        handleAddToCart(id, photoEl || imgWrap);
      });
...
if (grid) {
  try {
    ...
    console.error('Rendering products failed:', e);
    grid.innerHTML = '<p style="color:var(--muted);">Could not load products — try refreshing.</p>';
  }
}

function handleAddToCartLegacy(id) {
  // kept for any old inline onclick references
...
/* ==========================================================
   ...
   Full-size photo gallery with arrows/dots + full item info.
   ========================================================== */
...
if (modalQtyPlus) {
  modalQtyPlus.addEventListener('click', () => {
    modalState.qty = Math.min(modalState.maxQty, modalState.qty + 1);
    updateModalQtyUI();
  });
}
if (modalWishlistBtn) {
  modalWishlistBtn.addEventListener('click', () => {
    ...
    const nowSaved = toggleWishlist(modalState.id);
    ...
    // keep the card grid's heart in sync without a full re-render
    const cardBtn = grid.querySelector(`.wishlist-btn[data-id="${modalState.id}"]`);
    if (cardBtn) {
      cardBtn.classList.toggle('is-active', nowSaved);
      cardBtn.setAttribute('aria-pressed', String(nowSaved));
      cardBtn.innerHTML = nowSaved ? '&#9829;' : '&#9825;';
    }
  });
}
...
function openProductModal(id, startIndex) {
  ...
  if (typeof animateNumberTo === 'function') {
    animateNumberTo(modalPrice, formatPrice(product.price, product.currency), 450);
  } else {
    modalPrice.textContent = formatPrice(product.price, product.currency);
  }
...
if (typeof trapFocus === 'function') {
  trapFocus(productModal, () => productModal.classList.contains('is-open'));
}
...
/* ==========================================================
   9) CART DRAWER — instant slide-in cart instead of a full
   navigation to checkout.html. The drawer's own button still
   goes to checkout.html for address / GPS / payment details.
   ========================================================== */
...
if (cartDrawerContinueBtn) {
  cartDrawerContinueBtn.addEventListener('click', () => closeCartDrawer());
}
...
if (cartDrawer && typeof trapFocus === 'function') {
  trapFocus(cartDrawer, () => cartDrawer.classList.contains('is-open'));
}
