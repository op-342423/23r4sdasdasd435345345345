...
const { findValidCoupon, findAutoCoupon, incrementUsage, computeDiscount } = require('./coupons');
...
function serializeOrder(o, items) {
  return {
    ...
    paymentMethodKey: o.payment_method_key,
    paymentReference: o.payment_reference,
    paymentProofDataUrl: o.payment_proof_data_url,
...
//         paymentMethod, paymentReference, paymentProofDataUrl }
router.post('/', requireAuth, async (req, res) => {
  const {
    items, name, email, phone, address, notes, location, couponCode,
    paymentMethod, paymentReference, paymentProofDataUrl
  } = req.body;
  ...
  // Validate the chosen payment method against what the owner has
  // actually enabled — never trust the client's label for it, so a
  // customer can't submit an order claiming a method that's disabled
  // (or Visa/Mastercard, which are placeholders only for now).
  const requestedKey = (paymentMethod || 'cod').trim();
  let methodRow;
  try {
    const { rows } = await pool.query('SELECT * FROM payment_methods WHERE id=$1 AND enabled=TRUE', [requestedKey]);
    methodRow = rows[0];
  } catch (e) {
    console.error(e);
  }
  if (!methodRow) {
    return res.status(400).json({ error: 'That payment method is not available right now — please pick another.' });
  }
  if (methodRow.id === 'visa' || methodRow.id === 'mastercard') {
    return res.status(400).json({ error: 'Card payments are coming soon and can\u2019t be used yet.' });
  }
  ...
  try {
    ...
    if ((couponCode || '').trim()) {
      matchedCoupon = await findValidCoupon(couponCode, req.user.email);
      if (!matchedCoupon) {
        await client.query('ROLLBACK');
        return res.status(400).json({ error: "That coupon code isn't valid." });
      }
    } else {
      // No code typed — see if an auto-apply coupon covers this shopper.
      matchedCoupon = await findAutoCoupon(req.user.email);
    }
    if (matchedCoupon) {
      discount = computeDiscount(matchedCoupon, subtotal);
      couponLabel = matchedCoupon.label || matchedCoupon.code;
    }
    const deliveryFee = DELIVERY_FEE;
    const total = Math.max(0, subtotal - discount + deliveryFee);

    const { rows: orderRows } = await client.query(
      `INSERT INTO orders (user_id, name, email, phone, address, notes, location, coupon, subtotal, discount, delivery_fee, total, currency, payment_method, payment_method_key, payment_reference, payment_proof_data_url, status)
      ...
      [req.user.id, name, email || null, phone, address, notes || '', location ? JSON.stringify(location) : null, couponLabel, subtotal, discount, deliveryFee, total, currency,
        methodRow.name, methodRow.id, (paymentReference || '').trim() || null, paymentProofDataUrl || null]
    );
    ...
    if (matchedCoupon) await incrementUsage(matchedCoupon.id).catch(console.error);
