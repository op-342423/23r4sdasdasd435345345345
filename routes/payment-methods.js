...
router.put('/', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      // Media Library (Section 12) now exists — the admin dashboard's
      // "Choose from library" button on the QR upload sets
      // config.qrDataUrl to a library item's data URL by value, same
      // as a direct upload does. No schema change needed here either
      // way, since both paths just store a data URL string.
