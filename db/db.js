...
-- The storefront's nav icon moved from Instagram to TikTok. instagram_url
-- is kept as-is (no data loss) but is no longer read by the app; the new
-- column starts out copied from it (see the one-time migration in init()
-- below) so the owner has something to edit instead of a blank field.
...
-- Part 4 of the brief (this batch): the Media Library. A file
-- uploaded once here (product images, hero video, Learn More
-- backgrounds, collection/editorial media, payment QR codes) can be
-- reused everywhere via "Choose from library" instead of uploading
-- duplicates. Same base64-in-Postgres storage pattern already used
-- for every other image/video field in this schema -- data_url IS
-- the file. filename/size are display-only metadata.
...
-- Part 11 of the brief (this batch): the slim admin-controlled
-- announcement bar shown above the nav on every storefront page.
-- Single-row pattern, same as hero_video / site_settings.
...
-- Admin-managed discount coupons. Replaces the old hardcoded COUPONS
-- object in routes/orders.js. code is what the shopper types at
-- checkout (or, if auto_apply is on, it's applied for them with no
-- code needed). target_emails is a comma-separated allow-list — the
-- owner uses this to send a code privately to specific customers;
-- NULL/empty means anyone can use it. A coupon "expires automatically"
-- simply by falling outside [starts_at, expires_at] or hitting
-- usage_limit — it's never force-deleted by a background job, so the
-- owner can always see/re-enable it later; deleting it for good is a
-- separate, explicit action from the dashboard.
...
async function init() {
  ...
  // One-time migration for the Instagram → TikTok nav icon swap: if the
  // owner already had a link saved under the old field and hasn't set
  // the new one yet, carry it over as a starting point (they'll want to
  // edit it to their real TikTok URL from the admin dashboard).
  await pool.query(
    `UPDATE site_settings SET tiktok_url = instagram_url
     WHERE id = 1 AND tiktok_url IS NULL AND instagram_url IS NOT NULL`
  );
