  ...
  function renderImagePreview() {
    imagePreview.innerHTML = pendingImages.map((src, i) => `
      <div class="image-thumb" data-i="${i}">
        <img src="${src}" alt="${i === 0 ? 'Main product photo' : `Product photo ${i + 1}`}">
  ...
  try {
    ...
    listEl.innerHTML = products.map(p => {
      ...
      const thumb = images[0]
        ? `<img src="${images[0]}" class="admin-row__thumb" alt="${(p.name || '').replace(/"/g, '&quot;')}">`
...
async function setupSocialSettings() {
  ...
  try {
    ...
    tiktokInput.value = settings.tiktokUrl || '';
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    try {
      await saveSiteSettings({
        ...
        tiktokUrl: tiktokInput.value.trim(),
    ...
    listEl.innerHTML = stories.map(s => `
      <div class="admin-row" data-id="${s.id}">
        <span style="display:flex;align-items:center;gap:0.6rem;">
          ${s.mediaDataUrl ? `<img src="${s.mediaDataUrl}" class="admin-row__thumb" alt="${(s.title || '').replace(/"/g, '&quot;')}">` : `<span class="admin-row__thumb admin-row__thumb--empty" style="display:inline-block;"></span>`}
  ...
  try {
    ...
    listEl.innerHTML = items.map(it => {
      ...
      const thumb = it.kind !== 'quote' && it.mediaDataUrl
        ? `<img src="${it.mediaDataUrl}" class="admin-row__thumb" alt="${(it.caption || '').replace(/"/g, '&quot;')}">`
        : `<span class="admin-row__thumb admin-row__thumb--empty" style="display:inline-block;"></span>`;
      return `
...
function setupCoupons() {
  ...
  const labelInput = document.getElementById('couponLabelInput');
  const typeInput = document.getElementById('couponTypeInput');
  const valueInput = document.getElementById('couponValueInput');
  const startInput = document.getElementById('couponStartInput');
  ...
  const emailsInput = document.getElementById('couponEmailsInput');
  const autoApplyInput = document.getElementById('couponAutoApplyInput');
  const showBannerInput = document.getElementById('couponShowBannerInput');
  const bannerTextField = document.getElementById('couponBannerTextField');
  const bannerTextInput = document.getElementById('couponBannerTextInput');
  const addBtn = document.getElementById('addCouponBtn');
  const listEl = document.getElementById('couponsAdminList');
  if (!addBtn) return;

  let editingId = null;

  showBannerInput.addEventListener('change', () => {
    bannerTextField.style.display = showBannerInput.checked ? '' : 'none';
  });
  ...
  function resetForm() {
    ...
    emailsInput.value = '';
    autoApplyInput.checked = false;
    showBannerInput.checked = false;
    bannerTextInput.value = '';
    bannerTextField.style.display = 'none';
    addBtn.textContent = 'Save coupon';
  }

  // <input type="datetime-local"> wants "YYYY-MM-DDTHH:mm" in local
  // time with no timezone suffix — the API gives back a full ISO
  // timestamp, so convert in both directions around that boundary.
  ...
  addBtn.addEventListener('click', async () => {
    ...
    if (!code) { showToast('Give the coupon a code.', 'error'); return; }
    if (!value || value <= 0) { showToast('Enter a valid discount amount.', 'error'); return; }
    if (showBannerInput.checked && !bannerTextInput.value.trim()) {
      showToast('Add banner text, or turn the banner option off.', 'error');
      return;
    }
    ...
    const payload = {
      ...
      showBanner: showBannerInput.checked,
      bannerText: bannerTextInput.value.trim(),
      targetEmails: emailsInput.value.trim(),
      startsAt: startInput.value ? new Date(startInput.value).toISOString() : null,
      expiresAt: expiryInput.value ? new Date(expiryInput.value).toISOString() : null,
      usageLimit: usageLimitInput.value ? parseInt(usageLimitInput.value, 10) : null,
      enabled: true
    };
    ...
    try {
      ...
      resetForm();
      await renderCouponsAdminList();
    } catch (e) {
      showToast(e.message || 'Could not save the coupon.', 'error');
    } finally {
      addBtn.disabled = false;
    }
  });

  async function renderCouponsAdminList() {
    let coupons;
    try {
      coupons = await getCoupons();
    } catch (e) {
      listEl.innerHTML = '<p style="color:var(--muted);font-size:0.85rem;">Could not load coupons.</p>';
      return;
    }
    if (!coupons.length) {
      listEl.innerHTML = '<p style="color:var(--muted);font-size:0.85rem;">No coupons yet.</p>';
      return;
    }
    listEl.innerHTML = '<div class="admin-list"></div>';
    const list = listEl.querySelector('.admin-list');
    coupons.forEach(c => {
      const row = document.createElement('div');
      row.className = 'admin-row';
      const amount = c.discountType === 'flat' ? `$${c.discountValue}` : `${c.discountValue}%`;
      const bits = [
        c.autoApply ? 'auto-applied' : 'code',
        c.showBanner ? 'banner on' : null,
        c.targetEmails ? 'targeted' : null,
        c.usageLimit ? `${c.usedCount}/${c.usageLimit} used` : (c.usedCount ? `${c.usedCount} used` : null)
      ].filter(Boolean).join(' · ');
      row.innerHTML = `
        <span>
          <strong>${c.code}</strong> — ${amount} off ${c.label ? `(${c.label})` : ''}
          <span style="color:${c.isLive ? 'var(--muted)' : 'var(--red)'};font-size:0.78rem;display:block;">
            ${c.isLive ? 'Live' : 'Not currently live'} · ${bits}
          </span>
        </span>
        <span style="display:flex;gap:0.4rem;">
          <button type="button" data-action="toggle" data-id="${c.id}">${c.enabled ? 'Disable' : 'Enable'}</button>
      ...
      row.querySelector('[data-action="toggle"]').addEventListener('click', async () => {
        try {
          await updateCoupon(c.id, {
            code: c.code, label: c.label, discountType: c.discountType, discountValue: c.discountValue,
            autoApply: c.autoApply, showBanner: c.showBanner, bannerText: c.bannerText,
            targetEmails: c.targetEmails, startsAt: c.startsAt, expiresAt: c.expiresAt,
            usageLimit: c.usageLimit, enabled: !c.enabled
          });
          await renderCouponsAdminList();
        } catch (e) {
          showToast(e.message || 'Could not update the coupon.', 'error');
        }
      });
      ...
      row.querySelector('[data-action="edit"]').addEventListener('click', () => {
        ...
        valueInput.value = c.discountValue;
        startInput.value = toLocalInputValue(c.startsAt);
        expiryInput.value = toLocalInputValue(c.expiresAt);
        usageLimitInput.value = c.usageLimit || '';
        emailsInput.value = c.targetEmails || '';
        autoApplyInput.checked = c.autoApply;
        showBannerInput.checked = c.showBanner;
        bannerTextInput.value = c.bannerText || '';
        bannerTextField.style.display = c.showBanner ? '' : 'none';
      ...
      row.querySelector('[data-action="delete"]').addEventListener('click', async () => {
        ...
        try {
          ...
          showToast(e.message || 'Could not delete the coupon.', 'error');
        }
      });
    });
  }
