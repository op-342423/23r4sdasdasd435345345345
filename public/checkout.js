...
function renderCart() {
  ...
  cartLinesEl.innerHTML = cart.map(item => {
    ...
    const thumb = images[0] ? `<img src="${images[0]}" class="cart-line__thumb" alt="${(product.name || '').replace(/"/g, '&quot;')}">` : `<span class="cart-line__thumb cart-line__thumb--empty"></span>`;
    return `
      <div class="cart-line">
...
applyCouponBtn.addEventListener('click', async () => {
  ...
  const originalLabel = applyCouponBtn.textContent;
  applyCouponBtn.textContent = 'Checking...';
  try {
    const coupon = await apiValidateCoupon(code);
    appliedCoupon = { code: coupon.code, type: coupon.discountType, value: coupon.discountValue, label: coupon.label || coupon.code };
    couponStatus.textContent = `Applied: ${appliedCoupon.label}`;
    couponStatus.classList.add('is-ready');
  } catch (e) {
    appliedCoupon = null;
    couponStatus.textContent = e.message || "That code isn't valid.";
    couponStatus.classList.remove('is-ready');
  }
  applyCouponBtn.disabled = false;
  applyCouponBtn.textContent = originalLabel;
  renderTotals();
});

// If the shopper is logged in and the owner has an auto-apply coupon
// that covers them (site-wide, or targeted to their email), apply it
// immediately with no code needed — unless they already typed one in.
(async function autoApplyCoupon() {
  try {
    const me = await apiMe();
    if (!me.loggedIn) return;
    const coupon = await apiGetAutoCoupon(me.email);
    if (!coupon || appliedCoupon) return;
    appliedCoupon = { code: coupon.code, type: coupon.discountType, value: coupon.discountValue, label: coupon.label || coupon.code };
    couponInput.value = appliedCoupon.code;
    couponStatus.textContent = `Applied automatically: ${appliedCoupon.label}`;
    couponStatus.classList.add('is-ready');
    renderTotals();
  } catch (e) { /* no auto coupon — checkout works exactly the same without one */ }
})();

/* ---------------- GPS map picker ----------------
   The old flow silently trusted navigator.geolocation's single
   reading and sent it straight to the order — but device/browser
   location (especially on desktop, or with a weak GPS lock) can be
   off by a lot, which was landing wrong addresses on real orders.
   Now the shopper always sees an actual map and drags/taps the pin
   onto the right spot themselves; the coordinates that get saved
   are whatever the pin is sitting on when they hit "Confirm", not
   whatever the device guessed. GPS is only used to pick a sensible
   starting point for the pin. */
...
const locationUseGpsBtn = document.getElementById('locationUseGpsBtn');
const locationPinAddress = document.getElementById('locationPinAddress');

const DEFAULT_MAP_CENTER = { lat: 30.0444, lng: 31.2357 }; // Cairo — only used if GPS is unavailable/denied and the shopper hasn't moved the pin yet
let map = null;
let marker = null;
let pinAddressDebounce = null;
...
function initMapIfNeeded(center) {
  if (map) return;
  map = L.map('locationMap', { zoomControl: true }).setView([center.lat, center.lng], 15);
...
function onPinMoved(lat, lng) {
  locationPinAddress.textContent = 'Locating address…';
  clearTimeout(pinAddressDebounce);
  pinAddressDebounce = setTimeout(async () => {
    const address = await reverseGeocode(lat, lng);
    locationPinAddress.textContent = address ? `Pinned: ${address}` : `Pinned at ${lat.toFixed(5)}, ${lng.toFixed(5)}`;
  }, 400);
}

function centerMapOnDeviceGps(fallbackToDefault) {
  if (!navigator.geolocation) {
    if (fallbackToDefault) { initMapIfNeeded(DEFAULT_MAP_CENTER); onPinMoved(DEFAULT_MAP_CENTER.lat, DEFAULT_MAP_CENTER.lng); }
    return;
  }
  navigator.geolocation.getCurrentPosition(
    (position) => {
      const center = { lat: position.coords.latitude, lng: position.coords.longitude };
      initMapIfNeeded(center);
...
function openLocationModal() {
  ...
  // Give the modal a frame to become visible before Leaflet measures
  // its container — otherwise the map can render at 0×0.
  requestAnimationFrame(() => {
    if (!map) {
      const start = capturedLocation ? { lat: capturedLocation.lat, lng: capturedLocation.lng } : DEFAULT_MAP_CENTER;
      initMapIfNeeded(start);
      if (capturedLocation) {
        locationPinAddress.textContent = capturedLocation.address ? `Pinned: ${capturedLocation.address}` : `Pinned at ${start.lat.toFixed(5)}, ${start.lng.toFixed(5)}`;
      } else {
        centerMapOnDeviceGps(true);
      }
    } else {
      map.invalidateSize();
    }
...
if (gpsBtn) gpsBtn.addEventListener('click', openLocationModal);
...
if (locationModal) locationModal.addEventListener('click', (e) => { if (e.target === locationModal) closeLocationModal(); });
if (locationUseGpsBtn) {
  locationUseGpsBtn.addEventListener('click', () => {
    locationPinAddress.textContent = 'Getting your GPS location…';
    centerMapOnDeviceGps(false);
  });
}
if (locationConfirmBtn) {
  locationConfirmBtn.addEventListener('click', async () => {
    if (!marker) return;
    const pos = marker.getLatLng();
    capturedLocation = { lat: pos.lat, lng: pos.lng, accuracy: null, address: null };
    gpsStatus.innerHTML = `Location pinned — <a href="${mapsLinkFor(pos.lat, pos.lng)}" target="_blank" rel="noopener">view on map</a>`;
    ...
    if (resolvedAddress && capturedLocation) {
      ...
      gpsStatus.innerHTML = `Pinned: ${resolvedAddress} — <a href="${mapsLinkFor(pos.lat, pos.lng)}" target="_blank" rel="noopener">view on map</a>`;
    }
  });
}
