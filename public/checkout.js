...
document.getElementById('placeOrderBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    const paymentNote = order.paymentMethodKey === 'cod' || !order.paymentMethodKey
      ? `You'll pay in cash at the door when it arrives at:<br>${address}${capturedLocation ? ' (GPS location attached)' : ''}`
      : `Paying via <strong>${methodLabel}</strong>${order.paymentReference ? ` — reference: ${order.paymentReference}` : ''}. Delivering to:<br>${address}${capturedLocation ? ' (GPS location attached)' : ''}`;
