...
(function () {
  ...
  // Safety net for a slow-waking backend (e.g. a free-tier host's cold
  // start): if runIntro()'s 1.4s fallback fires before the real brand
  // name arrives, buildIntro() below locks in whatever text was on
  // screen at that moment (splitting it into letter spans and dropping
  // data-brand so brand.js never overwrites it again). Without this,
  // that stale text — usually the shipped "THORN" default — stays
  // wrong for the rest of the session even after the real name loads
  // seconds later. This listener runs every time brand.js applies a
  // name (cached-immediately or after however many retries) and
  // corrects heroTitle directly if it no longer matches.
  document.addEventListener('thorn:brandready', (e) => {
    const realName = e && e.detail && e.detail.brandName;
    if (!heroTitle || !realName) return;
    const current = (heroTitle.textContent || '').replace(/\s+/g, '');
    if (!heroTitle.hasAttribute('data-brand') && current !== realName.replace(/\s+/g, '')) {
      heroTitle.setAttribute('aria-label', realName);
      heroTitle.textContent = realName;
    }
  });
