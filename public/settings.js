...
router.post('/', requireAdmin, async (req, res) => {
  let { facebookUrl, instagramUrl, phone } = req.body;
  ...
  try {
    await pool.query(
      `INSERT INTO site_settings (id, facebook_url, instagram_url, phone)
       VALUES (1,$1,$2,$3)
       ON CONFLICT (id) DO UPDATE SET facebook_url=$1, instagram_url=$2, phone=$3`,
      [facebookUrl || null, instagramUrl || null, phone || null]
    );
    res.json(serialize({ facebook_url: facebookUrl, instagram_url: instagramUrl, phone }));
  } catch (e) {
    console.error(e);
