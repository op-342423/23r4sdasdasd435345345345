...
function showGalleryImage(i) {
  ...
  mainImage.alt = PRODUCT ? `${PRODUCT.name} — photo ${galleryIndex + 1} of ${GALLERY_IMAGES.length}` : '';
...
function renderGallery(images) {
  ...
  showGalleryImage(0);
  const name = PRODUCT ? PRODUCT.name : 'THORN product';
  thumbsEl.innerHTML = GALLERY_IMAGES.map((src, i) => `
    <button type="button" class="pdp__thumb${i === 0 ? ' is-active' : ''}" data-index="${i}" aria-label="Show photo ${i + 1} of ${name}">
...
function relatedCardHTML(p) {
  ...
  const photo = images[0]
    ? `<img class="product-card__photo" src="${images[0]}" alt="${(p.name || '').replace(/"/g, '&quot;')}" loading="lazy">`
