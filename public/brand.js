...
(function () {
  ...
  // Retry schedule for a cold-starting backend (e.g. a free-tier host
  // that takes several seconds to wake up on the first request after
  // being idle). Previously, one failed/timed-out fetch gave up for
  // good and left the shipped "THORN" default on screen for the rest
  // of the session — this keeps trying in the background so the real
  // name still appears the moment the server actually responds,
  // without making a normal (already-fast) load wait any longer.
  const RETRY_DELAYS_MS = [1500, 3000, 6000, 10000, 15000, 20000, 20000, 20000];

  function fetchSettings(attempt) {
    const ctrl = (typeof AbortController !== 'undefined') ? new AbortController() : null;
    const tId = ctrl ? setTimeout(() => ctrl.abort(), attempt === 0 ? FETCH_TIMEOUT_MS : 8000) : null;

    fetch('/api/settings', ctrl ? { signal: ctrl.signal } : undefined)
      .then((r) => (r.ok ? r.json() : null))
      .then((settings) => {
        if (tId) clearTimeout(tId);
        if (!settings) throw new Error('bad response');
        const brandName = settings.brandName || DEFAULT_BRAND_NAME;
        const heroTagline = settings.heroTagline || DEFAULT_HERO_TAGLINE;
        try {
      ...
      .catch(() => {
        ...
        if (attempt < RETRY_DELAYS_MS.length) {
          setTimeout(() => fetchSettings(attempt + 1), RETRY_DELAYS_MS[attempt]);
          return;
        }
        // Out of retries (~20s total): show whatever we've got so the
        // page isn't left in limbo, but resolving __brandReady here
        // doesn't stop later successes from still correcting the text
        // if the backend wakes up after this point.
        if (!cached) applyToDom(DEFAULT_BRAND_NAME, DEFAULT_HERO_TAGLINE);
      });
  }
  fetchSettings(0);
})();
