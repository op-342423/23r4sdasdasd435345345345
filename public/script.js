...
/* ==========================================================
   1) SPLIT-OPEN INTRO
   A stronger, fashion-film style entrance: the brand mark
   punches in and holds, a camera-flash washes the screen, then
   the two panels tear open to reveal the hero video underneath.
   ========================================================== */
(function runIntro() {
  const intro = document.getElementById('splitIntro');
  if (!intro) return;
  const start = () => {
    setTimeout(() => intro.classList.add('is-flashing'), 1150);
    setTimeout(() => intro.classList.add('is-open'), 1400);
    setTimeout(() => intro.remove(), 2700); // fully out of the way, incl. for a11y
  };
...
(async function setupSocialIcons() {
  ...
  let settings = { facebookUrl: '', instagramUrl: '', phone: '' };
  try { settings = await getSiteSettings(); } catch (e) { console.error('Could not load site settings:', e); }
