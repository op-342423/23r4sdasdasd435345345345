...
/* ==========================================================
   ...
   Facebook + TikTok open the owner's page in a new tab if
...
function loadSiteSettings() {
  if (!__siteSettingsPromise) __siteSettingsPromise = getSiteSettings().catch(() => ({ facebookUrl: '', tiktokUrl: '', phone: '', aboutText: '' }));
  return __siteSettingsPromise;
}

(async function setupSocialIcons() {
  const fbBtn = document.getElementById('socialFacebook');
  const ttBtn = document.getElementById('socialTiktok');
  const phoneBtn = document.getElementById('socialPhone');
  if (!fbBtn && !ttBtn && !phoneBtn) return;
  ...
  if (ttBtn) ttBtn.addEventListener('click', () => {
    if (settings.tiktokUrl) window.open(settings.tiktokUrl, '_blank', 'noopener');
    else showToast('No TikTok page linked yet.', 'error');
  });
...
function productPhotoHTML(images, name) {
  if (!images.length) return `<div class="product-card__img product-card__img--empty"></div>`;
  const altText = (name || 'THORN product').replace(/"/g, '&quot;');
  return `<div class="img-skeleton"></div><img class="product-card__photo" src="${images[0]}" alt="${altText}" draggable="false" loading="lazy" decoding="async">`;
}

/* Placeholder cards shown in the grid while getProducts() is still
   in flight, so the page shows a shimmering skeleton instead of a
...
function renderGridSkeleton(count) {
  ...
  const card = `
    <div class="skeleton-card">
      ...
      <div class="skeleton-card__block skeleton-card__btn"></div>
    </div>
  `;
    ...
    return `
      <div class="product-card reveal" data-id="${p.id}" data-index="0">
        ...
        <div class="product-card__img-wrap">
          ${productPhotoHTML(images, p.name)}
          ${arrows}
          ${dots}
          <button type="button" class="quick-view-btn" data-id="${p.id}">Quick view</button>
        </div>
  ...
  grid.querySelectorAll('.product-card').forEach(card => {
    ...
    if (quickViewBtn) {
      quickViewBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        openProductModal(id, current);
      });
    }

    // Touch swipe (mobile) — hover-zoom above already covers desktop.
    // Only meaningful with more than one photo; a single quick swipe
    // moves one frame, same as tapping an arrow.
    if (imgWrap && images.length > 1) {
      ...
      imgWrap.addEventListener('touchend', (e) => {
        ...
        touchStartX = null;
        if (Math.abs(dx) < 30) return; // treat as a tap, not a swipe
...
function renderModalFrame() {
  ...
  modalImage.alt = modalName.textContent ? `${modalName.textContent} — photo ${index + 1} of ${images.length}` : '';
...
function openProductModal(id, startIndex) {
  ...
  if (detailsLink) detailsLink.href = `product.html?id=${id}`;
  modalDesc.textContent = product.description || '';
  modalDesc.style.display = product.description ? '' : 'none';

  const note = getStockNote(product);
  modalStock.textContent = note ? note.text : '';
  modalStock.className = 'stock-note' + (note ? ' ' + note.cls : '');
  ...
  if (cart.length === 0) {
    cartDrawerItems.innerHTML = `
      <div class="empty-state empty-state--compact">
        ...
        <span class="empty-state__text">Your cart is empty — let's find you something.</span>
        <button type="button" class="empty-state__btn" id="cartDrawerShopNowBtn">Shop now</button>
      </div>
    `;
    const shopNowBtn = document.getElementById('cartDrawerShopNowBtn');
    if (shopNowBtn) {
