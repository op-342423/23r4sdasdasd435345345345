...
/* ==========================================================
   1) BANG INTRO
   The brand mark punches in and holds, then everything hits at
   once: a hard flash, a screen-shake, a burst of shards flying
   outward, and the panels tearing open fast — one strong beat
   instead of a slow reveal.
   ========================================================== */
(function runIntro() {
  const intro = document.getElementById('splitIntro');
  const shardsWrap = document.getElementById('splitIntroShards');
  if (!intro) return;
  ...
  function spawnShards() {
    ...
    for (let i = 0; i < count; i++) {
      ...
      shard.className = 'split-intro__shard';
      const angle = Math.random() * 360;
      const dist = 140 + Math.random() * 220;
      const dx = Math.cos((angle * Math.PI) / 180) * dist;
      const dy = Math.sin((angle * Math.PI) / 180) * dist;
      const w = 6 + Math.random() * 10;
      const h = 22 + Math.random() * 46;
      const rot = Math.random() * 180;
      shard.style.width = w + 'px';
      shard.style.height = h + 'px';
      shard.style.transform = `translate(-50%, -50%) translate(${dx}px, ${dy}px) rotate(${rot}deg)`;
      shard.style.opacity = '0.95';
      shardsWrap.appendChild(shard);
      requestAnimationFrame(() => {
        shard.style.transform = 'translate(-50%, -50%) translate(0, 0) rotate(0deg)';
  ...
  const start = () => {
    ...
    setTimeout(() => intro.remove(), 2050); // fully out of the way, incl. for a11y
  };
  ...
  function openAboutModal() {
    ...
    loadSiteSettings().then(settings => {
      aboutModalText.textContent = settings.aboutText && settings.aboutText.trim()
        ? settings.aboutText
        : "THORN is a small drop-based label — pieces made in short runs, not made for everyone. More details coming soon.";
    });
  }
...
try {
  setTimeout(() => lightningBurst(3), 1450);
